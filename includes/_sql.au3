...
;
; MSDN ADO    <a href='http://msdn.microsoft.com/en-us/library/ms807027.aspx' class='bbc_url' title='External link' rel='nofollow external'>http://msdn.microsoft.com/en-us/library/ms807027.aspx</a>
;
;Method                Description
...
Func _SQL_Connect($ADODBHandle, $server, $db, $username, $password, $SQLAuth = True)
	...
	If $SQLAuth = True then
		...
		$ADODBHandle.Open("Driver={MySQL ODBC 3.51 Driver};SERVER=" & $server & ";PORT=5123 ;Database=regdata ;DATABASE=" & $db & ";uid=" & $username & ";pwd=" & $password & ";") ;<==Connect with required credentials"
	Else
...
Func _SQL_ErrFunc()
    ...
    $SQLErr = "err.description is: " & @TAB & $MSSQLObjErr.description & @CRLF & _
            ...
            "err.helpcontext is: " & @TAB & $MSSQLObjErr.helpcontext & @CRLF
